    ...
    function add_walk_details( $content ) {
        ...
        if ( $walk->exists() ) {
            ...
            if ($place) {
                ...
                $latlng = $walk_place->field('latitude') . " and " .  $walk_place->field('longitude');
            } else {
                $latlng = $walk->field('start_point_latitude') . ", " . $walk->field('start_point_longitude');
            }
            $content .= "<tr><td>Start point</td><td>" . $latlng . "<br>(You can enter this lat/long directly into Google Maps)</td></tr>";
            $content .= "<tr><td>Length (km)</td><td>" . $walk->field('distance') . "</td></tr>";
            $content .= "<tr><td>Ascent (metres)</td><td>" . $walk->field('ascent') . "</td></tr>";
            $content .= "<tr><td>Walking time (hourse)</td><td>" . $walk->field('walking_time') . "</td></tr>";
            $content .= "<tr><td>Total time (hourse)</td><td>" . $walk->field('total_time') . "</td></tr>";
            $content .= "<tr><td>Recommendations or Restrictions</td><td>" . $walk->field('restrictions') . "</td></tr>";
            $content .= "</table>\n\n<br/><br/><p><br/><br>";
            $content .= '<a class="wis-single-walk-button" href="'. $walk->field('walk_description_file') . '">Download walk description</a>';
            if ($walk->field('gps_track_file')) {
                $content .= '<a class="wis-single-walk-button" href="'. $walk->field('gps_track_file') . '">Download GPS file</a>';
            }
            if ($walk->field('photo')) {
